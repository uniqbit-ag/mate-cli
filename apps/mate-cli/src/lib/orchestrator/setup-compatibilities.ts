...
function getLockedPackageManagers(): string[] {
  return listSetupPackageManagerCompatibilities()
...
export function getDefaultSetupSelections(): SetupSelections {
  return {
    allowedAgents: listSetupProviderCompatibilities()
      .filter((entry) => entry.defaultSelected)
      .map((entry) => entry.agent),
    packageManagers: listSetupPackageManagerCompatibilities()
      .filter((entry) => entry.defaultSelected)
      .map((entry) => entry.packageManager),
    capabilities: listSetupCapabilityCompatibilities()
      .filter((entry) => entry.defaultSelected)
