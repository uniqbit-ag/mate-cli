...
| `mate studio`                                       | Serve a local, read-only page over the workflow and every registered companion.    |
...
specs, and a Workflow view presenting the prompts a change is driven by — one
card per prompt, copied by clicking it.
...
until interrupted. Every endpoint is read-only.
