...
description: Discover, select, and publish archived OpenSpec changes through `mate artifact finish`. Use when the user wants to publish, ship, or push archived artifacts and anchor each one with a dated revert tag.
...
Publish archived work deliberately: discover what is pending, let the user select it, confirm the side effects once, then run the deterministic finish pipeline for each selected change.

## Workflow

Archiving a change is a local OpenSpec operation and never publishes anything. Publication is this skill: an explicit selection followed by one `mate artifact finish` call per selected change. That CLI owns archiving, capability sync, the scoped commit, remote synchronization, the dated tag, the push, and conflict handoff — this skill only discovers, selects, confirms, sequences, and reports.
...
1. **Discover pending changes.**
   ...
   Run it from the companion repository. It returns every dated archive under `openspec/changes/archive/` that has no local finish marker:
...
2. **Present the entries as a numbered list.** One line per entry, showing `name`, `anchor`, and `path` from the JSON. If `count` is `0`, say so and stop — there is nothing to publish. Never pick an entry for the user, and never default to "the newest" or "all of them".
   ...
   An explicitly supplied name is the one exception: if the user named an exact change — a known push failure being retried, or an operator-directed recovery — use that name even when `pending` does not list it, and go straight to step 3 with just that name. If the finish command later reports that no such change exists, report the lookup failure; nothing was committed, tagged, or pushed.
...
5. **Publish each selected change in selection order.**
   ...
   Run it from the companion repository, once per selected change, sequentially — never in parallel, never batched into one call. Finish mutates only the companion repository; the linked working repository is capability-indexing context. Do not manually invoke `mate cap index`.
...
6. **Branch on each result's `status`.** For the exact field meanings, the resumed-finish contract, and the conflict recovery path, read [references/openspec.md](references/openspec.md).
   ...
   - **`conflict`** → stop. Do not invoke finish for any remaining selection. Report the conflicted paths and follow the recovery workflow in [references/openspec.md](references/openspec.md).
   - **`error`** → stop. Report the failing `step`, the `message`, and what `local` says still exists. Do not invoke finish for any remaining selection.
...
- **CRITICAL — confirm before the first finish call**: the commit, the tag, and the push happen together, so the step 4 confirmation gates all three. Never skip it, and never treat an earlier "publish it"-style request as standing consent for this turn.
- **CRITICAL — no manual publishing**: never hand-commit or hand-tag instead of the finish CLI. For a still-active change the finish pipeline applies delta specs itself (via `openspec archive`) — do not pre-apply them, or produce fails with "already exists". A change whose specs were already synced (e.g. via `openspec-sync-specs`) must be archived first; finish then resumes from the archive without re-applying delta specs.
...
- A locally existing tag does not prove the remote tag was pushed. An explicitly named change is always eligible for a finish retry.
...
- Publishing several changes is one user workflow, not one atomic Git transaction. Each finish is independently resumable; a later failure never rolls back an earlier success.
