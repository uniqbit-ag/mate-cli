...
1. Identify the Companion Repository that owns the session and read its
   `CONTEXT-MAP.md` before any domain analysis.
2. Resolve the primary Working Repository from its canonical Git remote or the
   ...
   identity.
3. Resolve the repository-relative Area. In a monorepo, the Area is the owning
   package root such as `packages/acme`; in a non-monorepo it is the exact
   repository-relative path, such as `docs` or `.`.
4. Select the one context-map entry matching that repository and Area, then
   load only its mapped context file. Never apply another repository's context.
...
context or ADR artifact under the Companion Repository artifact root. Never
write directly to the Working Repository or a reference repository. Create
directories lazily, preserve unrelated content, and report the exact companion
artifact path changed. Update the mapped context inline as each term is confirmed
rather than batching changes; create a missing context or ADR file only when it
is first needed.
