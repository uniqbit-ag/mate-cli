...
import { getRequiredPluginIds, withRequiredSelections } from "../tools/setup/policy";
...
export interface SetupSelectorItem {
  ...
  locked?: boolean;
  disabled?: boolean;
  /** Registered with policy "required": pre-selected, locked, and explained. */
  required?: boolean;
}
...
const SELECTABLE_PLUGIN_KINDS = new Set(["provider", "packageManager", "capability"]);

// Wizard items for distribution-registered plugins that have no builtin
// compatibility entry (e.g. an enterprise capability added via createMate).
function getDistributionSelectorItems(knownIds: Set<string>): SetupSelectorItem[] {
  return getActiveDistribution()
    .registry.getEntries()
    .filter(
      (entry) => SELECTABLE_PLUGIN_KINDS.has(entry.plugin.kind) && !knownIds.has(entry.plugin.id),
    )
    .map((entry) => ({
      kind: entry.plugin.kind as "provider" | "packageManager" | "capability",
      id: entry.plugin.id,
...
function markRequiredItems(items: SetupSelectorItem[]): SetupSelectorItem[] {
  const requiredIds = getRequiredPluginIds();
  return items.map((item) =>
    SELECTABLE_PLUGIN_KINDS.has(item.kind) && requiredIds.has(item.id)
      ? {
          ...item,
          required: true,
          description: `${item.description} Required by this distribution and cannot be disabled.`,
        }
      : item,
  );
}
...
export function getSelectableItems(
  ...
  const baseItems: SetupSelectorItem[] = [
  ...
  return markRequiredItems([
    ...baseItems,
    ...getDistributionSelectorItems(knownIds),
      ...
      return [
        {
          ...
          indicator: renderIndicator(selectedProviders.has(item.id) || item.required === true),
        },
      ];
    }
    if (item.kind === "capability") {
      const disabled = !item.required && disabledCapabilityIds.has(item.id);
      return [
        {
          key: `capability:${item.id}`,
          label: item.required ? `${item.label} (required)` : item.label,
          description: item.description,
          indicator: renderIndicator(selectedCapabilities.has(item.id) || item.required === true, {
            disabled,
          }),
          dimColor: disabled,
        },
...
export async function selectSetupCompatibilities(
  ...
  return new Promise<SetupSelections | null>((resolve, reject) => {
    ...
    let currentStep = 0;
    let activeIndex = 0;
    // Required plugins are pre-selected regardless of saved selections.
    const initialSelections = withRequiredSelections(options.initialSelections);
    let selectedProviders = new Set(initialSelections.allowedAgents);
    let selectedPackageManagers = new Set(initialSelections.packageManagers);
    let selectedCapabilities = new Set(initialSelections.capabilities.map((entry) => entry.name));
    let selectedOpenSpecSchema =
      initialSelections.selectedOpenSpecSchema ??
      getOpenSpecSchemaSelection(initialSelections.capabilities);
    let selectedGitMode = initialSelections.selectedGitMode;
    ...
    const onData = (chunk: string) => {
      ...
      if (action === "submit") {
        ...
        const builtinCapabilityIds = new Set(
          listSetupCapabilityCompatibilities().map((entry) => entry.capability.name),
        );
        doResolve({
          ...
          capabilities: applyOpenSpecSchemaSelection(
            [
              ...
              // Distribution-registered capabilities without a builtin entry.
              ...[...selectedCapabilities]
                .filter((id) => !builtinCapabilityIds.has(id))
                .map((id) => ({ name: id })),
            ],
            selectedOpenSpecSchema,
          ),
