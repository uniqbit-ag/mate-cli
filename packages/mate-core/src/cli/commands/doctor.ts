...
function renderEngineRequirement(
  config: FrameworkConfig,
  distributionName: string,
  currentVersion: string,
): string {
  const range = config.engines?.[distributionName];
  const result = checkEngineRequirement(config, distributionName, currentVersion);
  return result.ok
    ? `engines.${distributionName}: satisfied (requires ${range}, running ${currentVersion})`
    : `engines.${distributionName}: ${result.reason}`;
}
...
// silent when healthy.
function printRequiredPluginDrift(config: FrameworkConfig): void {
  const drift = getRequiredPluginDrift(config);
  if (drift.length === 0) return;
  printSection(
    "Required Plugin Drift",
    renderTable(
      ["Plugin", "Kind", "Issue"],
      drift.map((entry) => [entry.pluginId, entry.kind, entry.reason]),
    ),
  );
}
...
export async function runDoctorCommand(_argv: string[] = [], deps: DoctorDeps = {}): Promise<void> {
  ...
  if (state === "linked-working-repository" && companionPath && repositoryId) {
    ...
    printRequiredPluginDrift(config);
