...
describe("PluginRegistry", () => {
  ...
  test("constructor accepts { plugin, policy } entries and preserves the policy", () => {
    const plugin = makePlugin("locked");
    const reg = new PluginRegistry([{ plugin, policy: "required" }]);
    expect(reg.getAll().find((p) => p.id === "locked")).toBe(plugin);
    expect(reg.getPolicy("locked")).toBe("required");
  });

  test("register() preserves an explicit policy entry", () => {
    const reg = new PluginRegistry([]);
    reg.register({ plugin: makePlugin("ext"), policy: "required" });
    expect(reg.getPolicy("ext")).toBe("required");
  });

  test("bare plugin with defaultSelected true derives policy 'default'", () => {
    const plugin = { ...makePlugin("pre-selected"), defaultSelected: true };
    const reg = new PluginRegistry([plugin]);
    expect(reg.getPolicy("pre-selected")).toBe("default");
  });

  test("bare plugin with defaultSelected false derives policy 'optional'", () => {
    const reg = new PluginRegistry([makePlugin("opt-in")]);
    expect(reg.getPolicy("opt-in")).toBe("optional");
  });
});
...
describe("SetupContext — activeProviders", () => {
  ...
  test("engine computes activeProviders from enabled provider plugins", async () => {
    ...
    const disabledRtk = createRtkPlugin({ isRtkOnPath: () => false });
    await applySetupCompatibilities(
      root,
      {
        allowedAgents: ["claude"],
