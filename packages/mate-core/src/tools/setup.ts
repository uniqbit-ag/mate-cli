...
export function mateFolderReadme(): string {
  ...
  return [
    ...
    `| \`${n} claude\` / \`${n} opencode\` / \`${n} pi\` | Launch an allowed agent from a linked working repository |`,
    `| \`${n} doctor\` | Check current link state, installed tools, and active capabilities |`,
    ``,
    `Run \`${n} claude\`, \`${n} opencode\`, or \`${n} pi\` from any linked working repository directory.`,
    ``,
    `## Configuration`,
    ``,
