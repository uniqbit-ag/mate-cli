...
describe("Codex launch integration", () => {
  ...
  test("wraps Codex with Headroom only when the capability is active", async () => {
    ...
    await withPath(binDir, async () => {
      const adapter = new CodexAdapter();
      const wrapped = await adapter.prepareLaunch(makeContext([{ name: "headroom" }]), ["prompt"]);
      expect(wrapped.command).toBe("headroom");
      ...
      expect(wrapped.env.HEADROOM_TELEMETRY).toBe("off");
      expect(wrapped.env.HEADROOM_OUTPUT_SHAPER).toBe("1");

      const direct = await adapter.prepareLaunch(makeContext(), ["prompt"]);
      expect(direct.command).toBe("codex");
      expect(direct.args).not.toContain("wrap");
    });
  });
});
